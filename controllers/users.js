...
const updatedPassWordByEmail = async (req,res) => {
  const {email} = req.body
  const saltRounds = parseInt(process.env.SALT);
  const encryptedPassword = await bcrypt.hash("7799", saltRounds);
  const query = `UPDATE users SET password = $1 WHERE email = $2;`;
  pool
  .query(query, [encryptedPassword, email]) 
  .then((result)=>{
    res.status(200).json({
      success: true,
